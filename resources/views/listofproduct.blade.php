  ...
  <header class="site-header">
    <nav class="site-navbar" aria-label="Hlavná navigácia">
      ...
      <form method="GET" action="{{ route('products.index') }}" class="navbar-search" role="search">
        <img
          src="{{ asset('images/magnifying-glass.png') }}"
          alt=""
    ...
    <form method="GET" action="{{ route('products.index') }}">
      <section>
        ...
        <details open>
          ...
          @php
            $selectedSports = request('sport', []);

            if (!is_array($selectedSports)) {
                $selectedSports = [$selectedSports];
            }

            $selectedSports = collect($selectedSports)
                ->map(fn ($sport) => mb_strtolower(trim($sport), 'UTF-8'))
                ->toArray();
            ...
            @foreach($sports as $value => $label)
              <label class="filter-check">
                <input
                  ...
                  name="sport[]"
                  value="{{ $value }}"
                  {{ in_array($value, $selectedSports) ? 'checked' : '' }}
                />
                {{ $label }}
              </label>
            @endforeach
  ...
  <main>
    <header>
      @php
        ...
        if (request('sport')) {
            $sportTitle = request('sport');

            if (is_array($sportTitle)) {
                $sportTitle = collect($sportTitle)
                    ->map(fn ($sport) => ucfirst(mb_strtolower(trim($sport), 'UTF-8')))
                    ->implode(', ');
            } else {
                $sportTitle = ucfirst(mb_strtolower(trim($sportTitle), 'UTF-8'));
            }

            $title = $sportTitle;
        } elseif (request('category') && request('subcategory')) {
